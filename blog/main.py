...
@app.get('/blogs', status_code=status.HTTP_200_OK, response_model=List[schemas.BlogResource], tags=['Blog'])
def blogs(db: Session = Depends(get_db)):
    return db.query(models.Blog).all()


@app.get('/blog/{id}', status_code=status.HTTP_200_OK, response_model=schemas.BlogResource, tags=['Blog'])
...
@app.post('/user', status_code=status.HTTP_201_CREATED, response_model=schemas.UserResource, tags=['User'])
...
@app.get('/user', status_code=status.HTTP_200_OK, response_model=List[schemas.UserResource], tags=['User'])
def show_all(db: Session = Depends(get_db)):
    return db.query(models.User).all()


@app.get('/user/{user_id}', status_code=status.HTTP_200_OK, response_model=schemas.UserResource, tags=['User'])
